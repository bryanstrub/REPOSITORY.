      ...
      download_main <- function(){
        ...
        # stocks <- stocks[466:468,] # just for dev
        ...
        for(id in stocks$idcaps){
          # print(id)
          tryCatch(download_stock(id), warning=function(w) cat("\n problem values: ", id, "\n"))
          # download_stock(id)
      ...
      download_stock <- function(id){
        ...
        if(is.na(page)){return()} # some stocks dont have historical price pages on yahoo?
      	...
      	if(page == ""){return()} # some stocks dont have historical price pages on yahoo?
      	...
      	names(page)[1] <- "value"
      	page <- page[!grepl("SPLIT",page$value),] %>% as.data.frame # why does this return a vector and I have to change it back to a data frame....
      	names(page)[1] <- "value"
      	...
      	bindme <- lapply(names(page),function(n){
      	  n = "V2"
      		...
      		temp <- gsub("\\}","",temp)
      		temp <- matrix(unlist(strsplit(temp,":")),ncol = 2, byrow = TRUE)[,2]
      	...
      	if (TRUE %in% is.na(page)){return()} # really werid splitting values were all rows are 0
